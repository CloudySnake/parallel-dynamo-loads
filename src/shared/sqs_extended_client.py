...
class SQSClientExtended(object):
    ...
    def receive_message(
            self, queue_url, max_number_Of_Messages=1, wait_time_seconds=10
    ):
        """
        ...
        for message in opt_messages:
            ...
            if large_pay_load_attribute_value:
                try:
                    ...
                    if (
                            "s3BucketName" not in message_body
                            and "s3Key" not in message_body
                    ):
                        raise ValueError(
    ...
    def __delete_message_payload_from_s3(self, receipt_handle):
        try:
            ...
            s3_object.delete()
            print(
                "Deleted s3 object https://s3.amazonaws.com/{}/{}".format(
                    s3_msg_bucket_name, s3_msg_key
                )
            )
        except Exception as e:
            print(
                "Failed to delete the message content in S3 object. {}, type:{}".format(
    ...
    def __is_s3_receipt_handle(self, receipt_handle):
        return (
            True
            if SQSExtendedClientConstants.S3_BUCKET_NAME_MARKER.value in receipt_handle
